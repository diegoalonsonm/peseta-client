...
const EditIncome = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.NEXT_PUBLIC_API_URL}/incomes/single/${incomeId}?email=${email}`)
      .then((res) => {
      ...
      .catch((err) => {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'No se pudo cargar el ingreso. Es posible que no exista o no tengas permiso para editarlo.'
        }).then(() => {
          router.push('/')
        })
      })
  }, [incomeId, email, router])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.put(`${process.env.NEXT_PUBLIC_API_URL}/incomes/${incomeId}`, data)
      .then((res) => {
        if (res.data.success) {
          Swal.fire({
            icon: 'success',
            title: 'Ingreso actualizado',
            text: 'El ingreso ha sido actualizado exitosamente',
            timer: 2000,
      ...
      .catch((err) => {
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: err.response?.data || 'Hubo un error actualizando el ingreso'
        })
      })
      .finally(() => {
      ...
      <div className="container">
        <div className="row text-center mt-5">
          <div className="col">
            ...
            <p className="mt-3">Cargando ingreso...</p>
          </div>
        </div>
      </div>
    )
